...
type ItemCalculado = { peca: PecaVenda; quantidade: number; valorUnitario: number; desconto: number; total: number }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [{ data: pecas, error: pecasError }, { data: clientes, error: clientesError }, { data: vendas, error: vendasError }] = await Promise.all([
      supabase.from('pecas').select('id, codigo, descricao, categoria, marca, valor_custo, valor_venda, estoque, ativo').eq('ativo', true).order('descricao'),
      supabase.from('clientes').select('id, nome, cpf_cnpj, whatsapp').order('nome').limit(1000),
      supabase.from('vendas').select('*, clientes:cliente_id(nome, cpf_cnpj)').order('criado_em', { ascending: false }).limit(100),
    ])
...
export async function POST(request: NextRequest) {
  ...
  const supabase = db(); const ids = [...new Set(itensBody.map((i: { pecaId?: unknown }) => Number(i.pecaId)).filter(Boolean))]
  const { data: pecas, error: pecasError } = await supabase.from('pecas').select('id, codigo, descricao, valor_custo, valor_venda, estoque, ativo').in('id', ids)
  ...
  try {
    itens = itensBody.map((entrada: { pecaId?: unknown; quantidade?: unknown; valorUnitario?: unknown; desconto?: unknown }) => {
      const peca = mapa.get(Number(entrada.pecaId)); const quantidade = numero(entrada.quantidade); const valorUnitario = dinheiro(entrada.valorUnitario ?? peca?.valor_venda); const desconto = dinheiro(entrada.desconto)
      if (!peca || peca.ativo === false || quantidade <= 0 || quantidade > numero(peca.estoque) || valorUnitario < 0 || desconto < 0) throw new Error(`Item ou estoque inválido: ${peca?.descricao ?? entrada.pecaId}`)
      const bruto = arredondar(quantidade * valorUnitario); if (desconto > bruto) throw new Error(`Desconto inválido em ${peca.descricao}.`)
      return { peca, quantidade, valorUnitario, desconto, total: arredondar(bruto - desconto) }
    })
  } catch (error) {
  ...
  try {
    const { data: venda, error: vendaError } = await supabase.from('vendas').insert({ numero_venda: numeroVenda, cliente_id: Number(body?.clienteId) || null, unidade_id: null, subtotal, desconto: descontoVenda + itens.reduce((a, i) => a + i.desconto, 0), total, forma_recebimento: forma, status: 'PAGO', observacao: texto(body?.observacao) || null, criado_por_nome: auth.nome, criado_por_email: auth.email }).select('id, numero_venda').single()
    if (vendaError) throw vendaError
    const { error: itensError } = await supabase.from('venda_itens').insert(itens.map((i) => ({ venda_id: venda.id, peca_id: i.peca.id, descricao: i.peca.descricao, codigo: i.peca.codigo, quantidade: i.quantidade, valor_custo_unitario: dinheiro(i.peca.valor_custo), valor_unitario: i.valorUnitario, desconto: i.desconto, total_item: i.total }))); if (itensError) throw itensError
    for (const item of itens) { const anterior = numero(item.peca.estoque); const posterior = anterior - item.quantidade; const { data: atualizada, error } = await supabase.from('pecas').update({ estoque: posterior }).eq('id', item.peca.id).eq('estoque', anterior).select('id').maybeSingle(); if (error || !atualizada) throw error ?? new Error(`Estoque de ${item.peca.descricao} foi alterado por outra operação.`); processados.push({ id: item.peca.id, estoque: anterior }); const { error: movError } = await supabase.from('pecas_movimentacoes').insert({ peca_id: item.peca.id, venda_id: venda.id, tipo: 'SAIDA_VENDA', quantidade: item.quantidade, estoque_anterior: anterior, estoque_posterior: posterior, observacao: `${numeroVenda} • ${ator}` }); if (movError) throw movError }
  ...
  } catch (error) { for (const item of processados) await supabase.from('pecas').update({ estoque: item.estoque }).eq('id', item.id); await supabase.from('vendas').delete().eq('numero_venda', numeroVenda); return NextResponse.json({ error: mensagem(error, 'Erro ao finalizar venda.') }, { status: 500 }) }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const supabase = db(); const { data: venda } = await supabase.from('vendas').select('id, numero_venda, status, total').eq('id', id).maybeSingle(); if (!venda || venda.status === 'CANCELADA') return NextResponse.json({ error: 'Venda não localizada ou já cancelada.' }, { status: 400 })
    ...
    for (const item of itens ?? []) { const { data: peca } = await supabase.from('pecas').select('estoque').eq('id', item.peca_id).maybeSingle(); const anterior = numero(peca?.estoque); const posterior = anterior + numero(item.quantidade); const { error } = await supabase.from('pecas').update({ estoque: posterior }).eq('id', item.peca_id); if (error) throw error; await supabase.from('pecas_movimentacoes').insert({ peca_id: item.peca_id, venda_id: id, tipo: 'ENTRADA_CANCELAMENTO_VENDA', quantidade: item.quantidade, estoque_anterior: anterior, estoque_posterior: posterior, observacao: `${venda.numero_venda} cancelada • ${ator}` }) }
    const { error } = await supabase.from('vendas').update({ status: 'CANCELADA', cancelado_por_nome: auth.nome, cancelado_por_email: auth.email, cancelado_em: new Date().toISOString(), cancelamento_motivo: motivo }).eq('id', id); if (error) throw error
