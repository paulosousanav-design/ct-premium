...
export default function AcademiaTecnicoPage() {
  ...
  return <main className="min-h-screen bg-[#c7d3cf] px-4 py-5"><div className="mx-auto max-w-6xl space-y-4">
    ...
    {selecionado && <section className="rounded-2xl bg-white p-5 shadow-sm"><button onClick={() => setSelecionado(null)} className="mb-3 text-sm font-black text-orange-600">← Voltar aos conteúdos</button><div className="flex flex-wrap gap-2"><Etiqueta>{rotuloTipo(selecionado.tipo)}</Etiqueta>{selecionado.obrigatorio && <Etiqueta>Leitura obrigatória</Etiqueta>}</div><h2 className="mt-3 text-2xl font-black text-slate-950">{selecionado.titulo}</h2>{selecionado.resumo && <p className="mt-2 text-slate-600">{selecionado.resumo}</p>}{selecionado.conteudo && <div className="mt-5 whitespace-pre-wrap rounded-xl bg-slate-50 p-5 text-sm leading-7 text-slate-800">{selecionado.conteudo}</div>}<Video url={selecionado.video_url} />{selecionado.arquivo_url && <a href={selecionado.arquivo_url} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-black text-white"><span aria-hidden="true">▣</span>Abrir PDF ou material</a>}<AssinaturaAcademia /><div className="mt-5 border-t pt-4">{progressoDe(progressos, selecionado.id)?.confirmado_em ? <p className="font-black text-emerald-700">✓ Conteúdo confirmado em {dataHora(progressoDe(progressos, selecionado.id)?.confirmado_em)}</p> : <button disabled={salvando} onClick={() => void registrar(selecionado.id, 'CONFIRMAR')} className="rounded-lg bg-emerald-600 px-5 py-3 font-black text-white disabled:opacity-50">Confirmar que li e compreendi</button>}</div></section>}
