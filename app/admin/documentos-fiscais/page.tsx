...
export default function DocumentosFiscaisPage() {
  ...
  async function manifestarCiencia(item: Documento) {
    if (!window.confirm(`Registrar Ciência da Operação para a NF-e de ${item.emitente_nome || 'fornecedor'}? A SEFAZ será acionada com o certificado da unidade para liberar o XML completo.`)) return
    await executar('MANIFESTAR_CIENCIA', { id: item.id }, 'Ciência da operação registrada. A busca do XML completo foi iniciada.')
  }
    ...
    <div className="mx-auto max-w-[1500px] space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-200">
          ...
          {!loading && filtrados.map((item) => <DocumentoLinha key={item.id} item={item} processando={processando} tratar={tratar} manifestarCiencia={manifestarCiencia} />)}
        </div>
      </section>
    </div>
  )
...
function DocumentoLinha({ item, processando, tratar, manifestarCiencia }: { item: Documento; processando: boolean; tratar: (id: number, acao: 'ARQUIVAR' | 'REABRIR' | 'IGNORAR') => Promise<void>; manifestarCiencia: (item: Documento) => Promise<void> }) {
  const completo = item.tipo_documento === 'NFE_COMPLETA'
  const tratado = ['ARQUIVADA', 'IGNORADA'].includes(item.status)
  return <article className="p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2"><Status valor={item.status} /><span className={`rounded-full px-2.5 py-1 text-xs font-black ${completo ? 'bg-blue-100 text-blue-800' : item.tipo_documento === 'RESUMO_NFE' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'}`}>{completo ? 'XML COMPLETO' : item.tipo_documento === 'RESUMO_NFE' ? 'RESUMO DA NF-E' : item.tipo_documento}</span></div><h3 className="mt-2 text-base font-black text-slate-950">{item.emitente_nome || item.descricao_evento || 'Documento fiscal eletrônico'}</h3><p className="mt-1 text-sm text-slate-600">{formatarCnpj(item.emitente_cnpj)} · Emissão {item.data_emissao ? data(item.data_emissao) : '-'} · <b>{moeda(Number(item.valor_total ?? 0))}</b></p><p className="mt-1 break-all font-mono text-[11px] text-slate-500">Chave: {item.chave_acesso || '-'} · NSU {item.nsu}</p>{item.tipo_documento === 'RESUMO_NFE' && <p className="mt-2 text-xs font-bold text-amber-700">A SEFAZ enviou somente o resumo. Registre a ciência para o sistema buscar o XML completo automaticamente.</p>}</div><div className="flex flex-wrap gap-2">{completo && item.status !== 'IMPORTADA' && <Link href={`/admin/pecas/importar-xml?dfe=${item.id}`} className="rounded-lg bg-orange-600 px-4 py-2 text-sm font-black text-white">Conferir e importar</Link>}{item.tipo_documento === 'RESUMO_NFE' && !tratado && <button disabled={processando} onClick={() => void manifestarCiencia(item)} className="rounded-lg bg-blue-700 px-3 py-2 text-xs font-black text-white disabled:opacity-50">Manifestar e buscar XML</button>}{!tratado && item.status !== 'IMPORTADA' && <><button disabled={processando} onClick={() => void tratar(item.id, 'ARQUIVAR')} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-black">Arquivar</button><button disabled={processando} onClick={() => void tratar(item.id, 'IGNORAR')} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-black text-red-700">Ignorar</button></>}{tratado && <button disabled={processando} onClick={() => void tratar(item.id, 'REABRIR')} className="rounded-lg border border-blue-200 px-3 py-2 text-xs font-black text-blue-700">Reabrir</button>}</div></div></article>
}
